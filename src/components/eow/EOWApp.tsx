...
export default function EOWApp() {
  ...
  const [reelsStatus, setReelsStatus] = useState<'idle' | 'recording' | 'done'>('idle')
  ...
  const handleRecordingDone = useCallback((blob: Blob) => {
    ...
    const file = new File([blob], `eow-${Date.now()}.webm`, { type: blob.type })

    // Try native share first (opens Instagram/TikTok/etc)
    ...
    setTimeout(() => setReelsStatus('idle'), 3000)
  }, [playingText])
    ...
    return (
      <div className="fixed inset-0 bg-[#0A0A0A] flex flex-col items-center justify-center px-6">
        <button
          ...
          className="absolute top-5 right-5 text-[#555] hover:text-[#999] text-[18px]"
        >
          ✕
        </button>
        ...
        <div className="w-full max-w-[400px] animate-fade-in">
          ...
          <div className="relative bg-[#F5F5F0] mx-auto">
            <div className="absolute -top-[6px] left-0 right-0 h-[6px] overflow-hidden">
              <svg width="100%" height="6" preserveAspectRatio="none" viewBox="0 0 400 6">
                <path d="M0,6 L0,3 Q10,0 20,3 Q30,6 40,3 Q50,0 60,3 Q70,6 80,3 Q90,0 100,3 Q110,6 120,3 Q130,0 140,3 Q150,6 160,3 Q170,0 180,3 Q190,6 200,3 Q210,0 220,3 Q230,6 240,3 Q250,0 260,3 Q270,6 280,3 Q290,0 300,3 Q310,6 320,3 Q330,0 340,3 Q350,6 360,3 Q370,0 380,3 Q390,6 400,3 L400,6 Z" fill="#F5F5F0" />
              </svg>
            </div>

            <div className="px-8 pt-10 pb-6">
              <p className="text-[10px] tracking-[0.25em] uppercase text-[#999] mb-6" style={fontJ}>
                End Of What
              </p>
              <p className="text-[20px] font-medium text-[#0A0A0A] leading-relaxed break-keep" style={{ fontFamily: 'var(--font-noto-serif), var(--font-cormorant), serif' }}>
                &ldquo;{playingText}&rdquo;
              </p>
            </div>
            ...
            <div className="absolute -bottom-[6px] left-0 right-0 h-[6px] overflow-hidden">
              <svg width="100%" height="6" preserveAspectRatio="none" viewBox="0 0 400 6">
                <path d="M0,0 L0,3 Q10,6 20,3 Q30,0 40,3 Q50,6 60,3 Q70,0 80,3 Q90,6 100,3 Q110,0 120,3 Q130,6 140,3 Q150,0 160,3 Q170,6 180,3 Q190,0 200,3 Q210,6 220,3 Q230,0 240,3 Q250,6 260,3 Q270,0 280,3 Q290,6 300,3 Q310,0 320,3 Q330,6 340,3 Q350,0 360,3 Q370,6 380,3 Q390,0 400,3 L400,0 Z" fill="#F5F5F0" />
              </svg>
            </div>
          </div>
          ...
          <div className="mt-10 flex flex-col gap-3">
            <button
              ...
              className="w-full py-3.5 rounded-full text-[10px] tracking-widest uppercase transition-all duration-200 active:scale-[0.98] bg-[#F5F5F0] text-[#0A0A0A]"
              style={fontJ}
            >
            ...
            {typeof navigator !== 'undefined' && 'share' in navigator && (
              <button
                onClick={() => navigator.share({ title: 'End Of What', text: `"${playingText}"\n\nso.now-then.dev/eow\n@jiwonnnnieee`, url: shareUrl }).catch(() => {})}
                className="w-full py-3 rounded-full text-[10px] tracking-widest uppercase transition-all duration-200 active:scale-[0.98] border border-[#F5F5F0]/30 text-[#F5F5F0]/60 hover:text-[#F5F5F0]"
                style={fontJ}
              >
                Share Link
              </button>
      ...
      <div className="fixed inset-0">
        ...
        {phase === 'done' && (
          <div className="fixed inset-0 z-[60] flex flex-col items-center justify-end pb-14 animate-fade-in">
            {reelsStatus === 'recording' && (
              <div className="mb-6 flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
                <span className="text-[10px] tracking-widest uppercase text-[#F5F5F0]/60" style={fontJ}>
                  Recording...
                </span>
              </div>
            )}
            <div className="flex flex-col items-center gap-3 w-full max-w-[320px] px-4">
              ...
              <button
                onClick={handleReels}
                disabled={reelsStatus === 'recording'}
                className="w-full py-3 rounded-full text-[10px] tracking-widest uppercase transition-all duration-200 active:scale-[0.98] bg-[#F5F5F0] text-[#0A0A0A] font-medium disabled:opacity-40"
                style={fontJ}
              >
                {reelsStatus === 'recording' ? 'Recording...' : reelsStatus === 'done' ? 'Done!' : 'Share to Reels'}
              </button>

              {/* Secondary row */}
              <div className="flex gap-3 w-full">
                <button onClick={() => play()} className="flex-1 py-2.5 rounded-full text-[10px] tracking-widest uppercase border border-[#F5F5F0]/30 text-[#F5F5F0]/60 hover:border-[#F5F5F0]/60 hover:text-[#F5F5F0] transition-all duration-200" style={fontJ}>Replay</button>
                <button onClick={resetToIdle} className="flex-1 py-2.5 rounded-full text-[10px] tracking-widest uppercase border border-[#F5F5F0]/30 text-[#F5F5F0]/60 hover:border-[#F5F5F0]/60 hover:text-[#F5F5F0] transition-all duration-200" style={fontJ}>New</button>
                <button onClick={() => setPhase('share')} className="flex-1 py-2.5 rounded-full text-[10px] tracking-widest uppercase border border-[#F5F5F0]/30 text-[#F5F5F0]/60 hover:border-[#F5F5F0]/60 hover:text-[#F5F5F0] transition-all duration-200" style={fontJ}>Link</button>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="flex flex-col items-center justify-center px-6 pt-16 pb-20">
        <div className="w-full max-w-[480px]">
          ...
          <div className="text-center">
            ...
            <p className="text-[9px] text-[#333] mt-3">Enter ↵</p>
          </div>
        </div>
      </div>
